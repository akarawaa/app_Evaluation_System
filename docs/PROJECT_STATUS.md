...
**2026-08-31 — frontend adopt shared design system (platform-core C5), branch `refactor/frontend-design-system` (ยังไม่ merge):**
...
**ยังไม่ทำ:** interior ของ `Evaluations`/`EvaluationDetail`/`People`/`Compare`/`Inbox`/`Tenants`/`TenantDetail` (ยังใช้ raw class — preset แค่ *เพิ่ม* token ของเก่าไม่พัง; สี slate/blue ตรงกับ token อยู่แล้วจึงกลมกลืน) — sweep ทีหลัง.
tsc + `npm run build` ผ่าน; Login verified (computed style: `bg-primary` #2563eb, `border-line`, `rounded-card` 12px, body Sarabun). หน้า authenticated ต้อง login (local dev = full stack) — review บน Vercel preview / production + Instant Rollback.
