...
- **การรับทราบของพนักงานแบบกระดาษ (employee acknowledgement) — schema + PDF + endpoint + UI ครบวงจร เสร็จ+พิสูจน์** (ระบบอีเมล/electronic เลื่อนเป็นเฟสถัดไปตามที่ตกลงกับผู้ใช้) →
  ...
  - Frontend: `EvaluationDetail.tsx` เพิ่ม section "การรับทราบของพนักงาน" (แสดงเฉพาะสถานะ `finalized`) — HR เห็นฟอร์มบันทึก (เลือกผล/วันที่/พยาน/ไฟล์แนบ/ความเห็นแย้ง) ถ้ายังไม่มีบันทึก, ทุกคนเห็นผลลัพธ์ถ้ามีแล้ว. `Evaluations.tsx` เพิ่มคอลัมน์ "การรับทราบ" ในตารางรายการ (รอรับทราบ/รับทราบแล้ว/รับทราบแล้ว (มีความเห็นแย้ง))
  ...
  - พิสูจน์: pytest 9 เคสใหม่ (`test_acknowledgement.py`: บันทึกสำเร็จ, ความเห็นแย้งยังนับว่ารับทราบ, ปฏิเสธไม่มีพยาน 400, บันทึกก่อน finalized 409, บันทึกซ้ำ 409, เฉพาะ HR ทำได้ (403 คนอื่น), ไฟล์แนบ upload/download ตรงกัน, ไม่มีไฟล์แนบ 404, list แสดงสถานะถูกต้อง) — **pytest 90/90** + browser จริง (login HR → เลือก "รับทราบ (มีความเห็นแย้ง)" + กรอกความเห็น → บันทึก → เห็นผลอัปเดตทั้งในหน้ารายละเอียดและหน้ารายการใบประเมิน) — **ทดสอบแค่ local, ยังไม่ deploy ขึ้น production** (migration 0018 เดิม deploy ไปแล้วรอบก่อน แต่ 0019 + endpoint/UI ชุดนี้ยังไม่ push — รอ decide ว่าจะ deploy รอบนี้เลยหรือรอ)
