...
**2026-08-31 — frontend adopt shared design system (platform-core C5), MERGED to master `08fc102`:**
`src/shared/ui` = vendor-copy จาก `platform-core/ui/` ผ่าน `sync-to.sh` (ไม่ใช่ submodule) + token preset `src/shared/tokens-preset.cjs`.
migrate แล้ว: **config** (tailwind preset / vite alias `@hr-suite/ui` / tsconfig paths / `index.css` base = `bg-canvas font-sans text-ink`),
**`AppHeader`** (โครงเดิม, restyle เป็น semantic token — `bg-surface`/`border-line`/`text-ink`/`text-primary`; ยังเป็น header ของแอปนี้เอง เพราะ nav หลายรายการ + CompanySwitcher/Badge ไม่พอดี `<AppShell>` แบบ leave/attendance),
**`ProtectedRoute`** (`<Spinner>`), **`CurrentUserBadge`/`CompanySwitcher`** (token), **auth pages** (`Login`/`ForgotPassword`/`ResetPassword`/`AuthHandoff` → `<Card>`+`<Button>`+token), **`Dashboard`** (`<Section>` + token).
**+ token sweep interior เสร็จ** (`3465c25`, 2026-08-31): `Evaluations`/`EvaluationDetail`/`People`/`Compare`/`Inbox`/`Tenants`/`TenantDetail` + `RequireRole` — 1:1 raw→token map (สีเดิมเป๊ะ ไม่มีการเปลี่ยนหน้าตา), tsc+build ผ่าน, Vercel redeploy. เก็บไว้: status colors (green/amber), `bg-slate-{100,200}`, `ring-blue-100`, bare `border`/`shadow`.
tsc + `npm run build` ผ่าน; Login verified (computed style: `bg-primary` #2563eb, `border-line`, `rounded-card` 12px, body Sarabun). Vercel redeploy `e-appraisal` — ตรวจหน้า authenticated บน production, เพี้ยนกด Instant Rollback (`537f466`).
...
**ล่าสุด:** **เปลี่ยนสูตรคะแนนการมา-ลาจากเชิงเส้น → ขั้นบันได (bracket) ตามเกณฑ์จริงของบริษัท** — ผู้ใช้ส่งรูปถ่ายเอกสาร HR จริง (`data/HR13380.jpg`, FMHR07 หน้า 4) พบว่าสูตรเดิม (`เต็ม − ค่าลด×จำนวนวัน`) ผิดตั้งแต่ต้น เพราะนโยบายจริงให้คะแนนเป็นช่วง (เช่น ลาป่วย 0-5 วันได้เต็ม 10 แล้วลดไม่เท่ากันเป็นขั้นๆ) — ดูรายละเอียดที่ "ทำไปแล้ว (ต่อ)" ด้านล่าง, ค่าเริ่มต้นถอดตรงจากภาพถ่าย ยกเว้น "สาย 0 ครั้ง" ที่ภาพไม่ชัด (ใส่ 10 ไว้ก่อนตามรูปแบบ 3 หมวดที่เหลือ — HR แก้ได้เองที่หน้า `/people` → ตั้งค่า ไม่ต้องรอรอบ deploy ถัดไป)
...
1. รอ HR: **ยืนยัน/แก้คะแนนหมวด "สาย" ช่วง 0 ครั้งที่หน้า `/people` → ตั้งค่า** (ภาพถ่ายเอกสารต้นฉบับไม่ชัดจุดนี้ ใส่ 10 ไว้ก่อนตามรูปแบบ 3 หมวดที่เหลือ — ดู "ทำไปแล้ว (ต่อ)" 2026-08-29) ส่วนอีก 3 หมวด (ลากิจ/ขาดงาน/ลาป่วย) ยืนยันตรงจากเอกสารจริงแล้วไม่ต้องรอ, ตรวจ/ปรับถ้อยคำ BARS anchors, เกณฑ์ probation ต่อ checkpoint — ส่งไฟล์ `exports/evaluation-criteria-bars.docx` ให้ตรวจแล้ว
...
- **สูตรคะแนนการมา-ลา: เชิงเส้น → ขั้นบันได (bracket) ตามเกณฑ์จริง (2026-08-29) เสร็จ+พิสูจน์แล้ว** —
  ผู้ใช้ส่งรูปถ่ายเอกสารจริง `data/HR13380.jpg` (FMHR07 หน้า 4, "เกณฑ์มาตรฐานการกำหนดคะแนน") พบว่า
  **สูตรเดิม (`0017_attendance_formula_settings.sql`: เต็ม − ค่าลด×จำนวนวัน) ไม่ตรงกับนโยบายจริงเลยตั้งแต่
  แรก** — บริษัทให้คะแนน 4 หมวด (ลากิจ/ขาดงาน/ลาป่วย/สาย) หมวดละ 10 คะแนนแบบ**ช่วง** ไม่ใช่หักต่อหน่วย
  เช่น ลาป่วย 0-5 วัน (มีใบรับรองแพทย์) ยังได้เต็ม 10 แล้วลดไม่เท่ากันเป็นขั้น (8→6→4→2→1→0) ซึ่งสูตรเชิงเส้น
  แทนไม่ได้เลยไม่ว่าจะตั้งค่าสัมประสิทธิ์เท่าไหร่
  - **`0023_attendance_brackets.sql`**: ตาราง `company_attendance_brackets` (company_id, category,
    min_value, max_value nullable=ไม่จำกัดบน, score, sort_order) — RLS 4 policy + grant ตาม pattern
    มาตรฐาน (non-negotiable #5). **ไม่ลบตาราง 0017 เดิม** (non-destructive — เผื่อมี tenant ตั้งค่าไว้แล้ว)
    แค่เลิกใช้งานจริง
  - **`services/attendance_brackets.py`** ใหม่ — `DEFAULTS` ถอดตรงจากภาพถ่าย 4 หมวด (คง `compute_score()`
    signature เดิมของ `attendance_formula.py` ไว้เป๊ะ ย้าย call site ได้โดยไม่ต้องแก้โครงสร้างเรียก),
    `_validate()` บังคับช่วงต่อเนื่องไม่มีช่องว่าง/ทับซ้อน เริ่มที่ 0 จบแบบไม่จำกัดบนเสมอ (400 ถ้าไม่ผ่าน),
    ช่วงที่ไม่ครอบคลุม (bug ในอนาคต) fail-safe ไปที่ 0 คะแนนแทนการเดา
  - **⚠️ ค่าที่ยังไม่ยืนยัน**: หมวด "สาย" ในภาพไม่เห็นแถว "0 ครั้ง = กี่คะแนน" (อาจถูกตัด/เบลอ) — ใส่ 10 ไว้ก่อน
    ให้ตรงรูปแบบ 3 หมวดที่เหลือ (ทุกหมวดที่เห็นครบ "ไม่เคย/ไม่มี" = 10 เสมอ) **HR ต้องเข้าไปยืนยัน/แก้ที่หน้า
    `/people` → ตั้งค่า** เอง ไม่ต้องรอ deploy รอบหน้า (แก้ได้ทันทีผ่าน UI)
  - ย้าย call site: `services/evaluations.py`'s `set_attendance()`, `services/attendance_import.py`'s
    bulk import — ทั้งคู่แค่เปลี่ยน `attendance_formula.get_formula/compute_score` →
    `attendance_brackets.get_brackets/compute_score` (shape เดิมเป๊ะ ไม่ต้องแก้ logic รอบข้าง)
  - Routes ใหม่: `GET /api/settings/attendance-brackets`, `PUT /api/settings/attendance-brackets/{category}`
    (ทีละหมวด ไม่ใช่ทั้ง 4 พร้อมกัน — HR แก้/save ทีละการ์ดได้อิสระ) — routes เดิมของ 0017 ยังอยู่ (ไม่ลบ)
    แต่ไม่มีผลต่อคะแนนจริงอีกต่อไป (เอกสารกำกับไว้ชัดในทั้ง route/service/test)
  - Frontend: `People.tsx` → แท็บ "ตั้งค่า" เปลี่ยนจากฟอร์ม 5 ตัวเลขเดิม → การ์ด 4 หมวด (`AttendanceBracketEditor`
    component ใหม่ในไฟล์เดียวกัน) แต่ละหมวดมีตารางช่วง (ตั้งแต่/ถึง/คะแนน) + เพิ่ม/ลบแถว + บันทึกแยกทีละหมวด
  - **พิสูจน์สดครบวงจรผ่าน browser จริง** (ไม่ใช่แค่ pytest): เปิดหน้าตั้งค่า → เห็นค่าเริ่มต้นตรงกับภาพถ่าย
    ทุกหมวด → แก้ลาป่วยช่วงแรกจาก 10 เป็น 9 → บันทึก → เช็ค DB ตรงว่าเปลี่ยนจริง → สร้างใบประเมิน + ตั้ง
    sick_days=2 ผ่าน API → **คะแนนออกมา 39 (9+10+10+10) ไม่ใช่ 40** ยืนยันว่าค่าที่แก้ feed เข้าคำนวณจริง
    ไม่ใช่แค่ persist เฉยๆ
  - แก้ 3 เทสต์เดิมที่ผูกกับผลลัพธ์สูตรเชิงเส้น (`test_attendance.py` 2 เคส,
    `test_attendance_formula.py` 1 เคส — เปลี่ยนเป็นยืนยันว่าแก้สูตรเดิมแล้ว**ไม่มีผล**ต่อคะแนนจริงอีกต่อไป
    แทน) + เพิ่ม `test_attendance_brackets.py` ใหม่ 10 เคส (defaults, RBAC, validation 5 แบบ
    ก่อน/ว่าง/ทับซ้อน, persist+feed compute, tenant isolation) — pytest ทั้งชุดผ่านหมด
...
- สูตรคะแนน attendance เป็นแบบขั้นบันได (bracket) ตามเอกสารจริง (FMHR07 p.4, `data/HR13380.jpg`) แล้ว 3/4 หมวด ยืนยันตรงจากภาพ — เหลือ "สาย 0 ครั้ง" ที่ภาพไม่ชัด (ใส่ 10 ไว้ก่อน) รอ HR ยืนยัน/แก้เองที่ `/people` → ตั้งค่า
